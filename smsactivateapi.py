...
class VOIPGenerator:
    def __init__(self, token):
        ...
        if (not token):
            ...
            if "BAD_KEY" in requests.get(
                                    "http://sms-activate.ru/stubs/handler_api.php?api_key=" + token + "&action=getBalance").text:
                raise APIError(
                    json.dumps({"ok": False, "error_code": 400, "error": "The api_key is invalid. Try with another!"}))
            else:
    ...
    def getNumber(self, service, country):
        value = requests.get(
            self.api_url + "action=getNumber&service=" + str(service) + "&ref=1222&country=" + str(country)).text
        if (not service):
    ...
    def getStatus(self, id_number):
        ...
        if ("NO_ACTIVATION" in value):
            ...
            if ("STATUS_OK" in value):
                return json.dumps(
                    {"ok": True, "status": value.replace(value.split("K")[1], ""), "code": value.split(":")[1]})
